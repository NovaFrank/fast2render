...
export default {
  name: 'SelectPurchaseOrganization',
  props: {
    ...
    addBtnText: { type: String, default: '选择采购组织' },
    ...
    api: {
      type: Function,
      default: () => {
        return ElsOrgInfoService.listPurchaseOrganization;
      }
    },
    // 表格的分页配置
    page: {
  ...
  methods: {
    handleList() {
      ...
      listParams.orgCategoryId = 'purchaseOrganization';
      listParams.currentPage = this.crudPage.currentPage;
      listParams.pageSize = this.crudPage.pageSize;
      ElsOrgInfoService.listPurchaseOrganization(this.actionPath, listParams).then((res) => {
        const data = res.data;
