<template>
  <div>
    ...
    <el-dialog :title="title" :visible.sync="visable" append-to-body @open="handleList">
      <avue-crud
        ref="crud"
        v-model="crudObj"
        :data.sync="crudData"
...
<script>
import { SupplierMasterDataService } from '../../../lib/api/materials';
import { getUserInfo } from '../../../lib/auth';
import supplierDialogOption from './const/supplierList';
...
export default {
  ...
  props: {
    ...
    addBtnText: { type: String, default: '选择供应商' },
    ...
    column: {
      type: Array,
      default: () => {
        return supplierDialogOption.option.column;
      }
    },
    // 表格的分页配置
    page: {
  ...
  methods: {
    handleList() {
      ...
      SupplierMasterDataService.list(this.actionPath, listParams).then((res) => {
        const data = res.data;
