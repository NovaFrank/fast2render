  ...
  <div :class="b()">
    ...
    >
      <div slot-scope="{ node, data }" :class="b('item')">
        ...
        >
          {{ data[labelKey] }}
          <div :class="b('menu')" v-show="data.isShow && vaildData(option.menu, true)">
...
<script>
import { DIC_PROPS } from './variable';
import { deepClone, vaildData } from '../../../lib/utils';
...
export default create({
  ...
  methods: {
    ...
    save(data, done) {
      const callback = () => {
        const form = deepClone(Object.assign(this.form, { isShow: false }));
        if (this.type === 'add') {
