  ...
  async created() {
    ...
    if (this.isEdit) {
      ...
      this.applicationTypeValue =
        this.$route.params && this.$route.params.id.split("_")[1];
      this.purchaseRequestNumber =
        this.$route.params && this.$route.params.id.split("_")[0];
  ...
  methods: {
    ...
    addMaterial() {
      ...
      if (this.dialogTitle === "选择物料") {
        ...
        convertExecute("307000", "307000", this.typeCode, stringifyData).then(
          res => {
            console.log(
              "res :",
    ...
    async selectChange(value) {
      ...
      getTablePageConfig("307000", this.typeCode).then(async res => {
        ...
        if (rows) {
          ...
          let price = await findPriceByMaterialNumber(
            "292000",
            "39001010001",
            "avgPrice"
          );
      ...
      // 动态表体
      findTableColumnConfig("307000", "1001", "", this.tableCode).then(
        async res => {
