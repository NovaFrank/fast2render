<template>
  <div class="detail-box">
    ...
    <fast2-head-provider
      ...
      @projectTypeChange="handleProjectTypeChange"
    ></fast2-head-provider>
...
export default {
  ...
  methods: {
    ...
    handleAddAnnex() {
      this.opreation = OPERATION.ADD_ANNEX;
    ...
    hideOpenForms(materialNumber) {
      // const formNameList = ['subtotalAmount'];
      // const avueFormElement = document.querySelector('.avue-dialog .avue-form');
      // if (avueFormElement) {
      //   for (const formName of formNameList) {
      //     const querySelector = `label[for="${formName}"]`;
      //     const labelElement = avueFormElement.querySelector(querySelector);
      //     if (labelElement) {
    ...
    resetForm(configFormValues, formColumns) {
      ...
      this.$nextTick(() => {
        ...
        Object.assign(this.form, this.defaultFormValue);
      });
    },
    ...
    saveRemote(params) {
      const saveMethod = this.opreation === OPERATION.RELEASE ? 'release' : 'create';
      ...
      PurchaseOrderService[saveMethod](params)
        .then((res) => {
          if (res.data && res.data.statusCode === '200' && res.data.message === 'response OK') {
            ...
            if (this.opreation === OPERATION.ADD_ANNEX) {
              this.$message.success('保存成功, 继续添加附件');
              // 不返回列表, 转到 detail 页面, 继续上传附件等其他操作
              const orderNumber = data.orderNumber;

              this.$router.push({ path: `${ROUTER_PATH_ORDER_V2}/edit/${orderNumber}` });
            } else if (this.opreation === OPERATION.SAVE) {
              // 返回列表
    ...
    selectedMaterails(materialList) {
      if (materialList.length > 0) {
        ...
        for (const material of addMaterialList) {
          ...
          for (const prop in showColumnProps) {
            item[prop] = material[prop] || '';
          }

          // 单位
          item.baseUnit = material.baseUnit || '';
          // if (material.cateLevelCode) {
          //   item.cateLevelCode = material.cateLevelCode;
          // }
          ...
          // 创建. 修改数据(新增的行将物料的uuid放入queryUuid字段), 重新选择了物料也要更新 queryUuid
          item.queryUuid = material.uuid;

          const materialObject = this.materialListOption.obj;

          Object.assign(materialObject, item);
        }
    ...
    setMaterialListProps() {
      const columns = this.materialListOption.option.column;

      const props = {};

      for (const column of columns) {
        props[column.prop] = column.label;
      }

      this.materialListProps = props;
    },

    setTableColumns(configObj) {
      this.materialListOption.data = [];

      this.showColumnProps = null;
      ...
      for (const item of tableColumns) {
        ...
        // if (item.label === '采购申请行号') {
        //   continue;
        // }

        // 新建时，删除单子号, 行号
        if (item.prop === 'orderNumber' || item.prop === 'orderItemNumber') {
          continue;
        }
        ...
        /*
        /* 订单 目前没有这个设置
        if (DEFAULT_COLUMN_ITEMS[columnItem.prop]) {
          Object.assign(columnItem, DEFAULT_COLUMN_ITEMS[columnItem.prop]);
        }
        */

        // 根据当前业务显示或其他要求，删除配置错误中存在的错误属性
        /* 订单 目前没有这个设置
        const excludeProps = MATERIAL_LIST_OPTION.exclude || {};

        for (const excludeProp in excludeProps) {
          delete columnItem[excludeProp];
        }
        */
...
.avue-group--header .el-collapse {
  border-top: 0;
  border-bottom: 0;
}
.detail-box .topForm .el-collapse-item__header {
  display: none;
}
.detail-box .topForm .avue-form__menu--center {
  display: none;
}
.detail-box .topForm .el-collapse-item__content {
  padding-bottom: 0;
}
</style>
